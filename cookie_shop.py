...
def get_cookie_from_dict(id, cookies):
    ...
    """
    # write your code for this function below this line
    return next(cookie for cookie in cookies if cookie['id'] == id)
...
def solicit_order(cookies):
    ...
    order = []
    while True:
        cookie_id = input("Please enter the ID of the cookie you would like to order, or type 'finished' to complete your order: ").strip().lower()
        if cookie_id in ['finished', 'done', 'quit', 'exit']:
            break
        if cookie_id.isnumeric():
            ...
            if any(cookie['id'] == cookie_id for cookie in cookies):
                ...
                print("That cookie ID does not exist. Please enter a valid cookie ID.")
        else:
...
def display_order_total(order, cookies):
    ...
    total_cost = 0
    for item in order:
        cookie = next(cookie for cookie in cookies if cookie['id'] == item['id'])
        total_cost += cookie['price'] * item['quantity']
